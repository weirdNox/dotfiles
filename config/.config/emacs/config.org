...
(use-package selectrum :straight t
  ...
  (setq! suggest-key-bindings nil
         selectrum-max-window-height nil)
  (selectrum-mode 1))
#+end_src

*** Filtering and frecency-based sorting
#+begin_src emacs-lisp
...
(use-package ctrlf :straight t
  :init
  ;; NOTE(nox): Workaround to prevent the `ctrl-mode-map' from being set at all times
  (define-globalized-minor-mode ctrlf-all-mode ctrlf-local-mode ctrlf-local-mode)
  (ctrlf-all-mode 1))
#+end_src
...
(use-package cc-mode
  ...
  (add-hook! c-mode (setq-local comment-start "// "
                                comment-end   ""))
...
(use-package matlab-shell
  ...
  (setq! matlab-shell-command-switches '("-nodesktop" "-nosplash")
         ...
         matlab-shell-input-ring-size 25000)

  (defun matlab-shell-first-prompt-fcn ())

  ;; NOTE(nox): Write history correctly
  (defun +matlab-shell|sentinel (process event)
    (when (not (process-live-p process)) (comint-write-input-ring)))

  (defun +matlab-shell*set-hooks ()
    (add-hook! :local 'kill-buffer-hook (comint-write-input-ring))
    (set-process-sentinel (get-buffer-process (current-buffer)) #'+matlab-shell|sentinel))
...
*** SPICE
#+begin_src emacs-lisp
(use-package spice-mode :straight t
  :mode (("\\.sp\\'"  . spice-mode)
         ("\\.cir\\'" . spice-mode)
         ("\\.cdl\\'" . spice-mode)
         ("\\.chi\\'" . spice-mode)
    ...
    (let ((capture-frame (cl-loop for frame in (frame-list)
      ...
      (select-frame-set-input-focus (or capture-frame
                                        (make-frame '((name . "Org Capture") (org-capture-frame . t)
                                                      (width . 110) (height . 40)))))
      (unless capture-frame (org-capture))))
...
"
  ...
  (embark-define-keymap embark-bibtex-map
    "Keymap for Embark bibtex actions."
...
;; NOTE(nox): C/C++
;;
(rx-let ((common (: (| (group-n 6 (| "warning"))
                       (group-n 7 (| "info" "information" "informational" "instantiated from" "required from" "note"))
                       (| (: (? "fatal ") "error") "sorry,"))
                    (| " " ":"))))
  (add-to-list 'compilation-error-regexp-alist 'cc-no-file)
  (add-to-list 'compilation-error-regexp-alist-alist `(cc-no-file ,(rx common) nil))

  (add-to-list 'compilation-error-regexp-alist 'cc)
...
;; NOTE(nox): NVCC
;;
(rx-let ((common (: (| (group-n 6 (| "warning")) "error") (* nonl) ":")))
  (add-to-list 'compilation-error-regexp-alist 'nvcc)
  (add-to-list 'compilation-error-regexp-alist-alist
               `(nvcc
                 ,(rx
                   bol
