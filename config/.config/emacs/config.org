...
(use-package dumb-jump :elpaca t
  ...
  (setq! dumb-jump-force-searcher 'rg
         ...
         dumb-jump-language-file-exts nil
         dumb-jump-find-rules nil)

  ;; Speed it up by filtering first and then searching just the relevant files
  (defun dumb-jump-generate-rg-command (look-for _cur-file proj regexes lang exclude-paths)
    ...
    (let* ((filled-regexes (dumb-jump-populate-regexes look-for regexes 'rg))
           ...
           (cmd (concat dumb-jump-rg-cmd
                        " -0l"
                        " " (shell-quote-argument (format "\\b%s\\b" (regexp-quote look-for)))
                        " " (shell-quote-argument proj)
                        " | xargs -0 "
                        dumb-jump-rg-cmd " --color never --no-heading --line-number -U"
      ...
      (if (= (length regexes) 0)
          ""
        (dumb-jump-concat-command cmd exclude-args regex-args)))))
#+END_SRC
               ...
               (cons 'any-parent-is
                     (lambda (type)
                       (lambda (_n parent &rest _)
                         (catch 'term
                           (while parent
                             (let ((match (string-match-p type (treesit-node-type parent))))
                               (when match (throw 'term match))
                     ...
                     (lambda (name &optional parent-type)
                       (lambda (node parent &rest _)
                         (and (or (null parent-type) (string-match-p parent-type (treesit-node-type parent)))
                              (string-match-p name (or (treesit-node-field-name-for-child parent (treesit-node-index node)) "")))))))
                       ...
                       (save-excursion
                         (catch 'term
                           (while parent
                             ...
                             (when (looking-back (rx bol (* whitespace)) (line-beginning-position)) (throw 'term (point)))
  ...
  (add-to-list 'treesit-simple-indent-presets
               (cons 'any-parent-nth-child ; NOTE(nox): To be used with any-parent-is
                     (lambda (type &optional child-idx)
                       (lambda (_n parent &rest _)
                         (let ((parent (catch 'term
                                         (while parent
                                           (when (string-match-p type (treesit-node-type parent)) (throw 'term parent))
                                           (setq parent (treesit-node-parent parent))))))
                           (treesit-node-start (treesit-node-child parent (or child-idx 0)))))))))
#+end_src
...
(use-package c-ts-mode
       ...
       (catch 'term
         (while parent
           (let ((ptype (treesit-node-type parent)))
             (if (string-match-p ptype "preproc")
                 (setq parent (treesit-node-parent parent))
  ...
  (defun +c-ts-mode--standalone-parent-or-uncle (_n parent &rest _)
    (save-excursion
      (catch 'term
        (while parent
          (while (string-match-p (treesit-node-type parent) "preproc")
            (setq parent (treesit-node-parent parent)))
          (goto-char (treesit-node-start parent))
  ...
  (defun +c-ts-mode--parenthesized-indent (search-for)
    `(lambda (child parent &rest _)
       (cl-loop for is-first = (= (treesit-node-index child) 1) then nil
                until (string= ,search-for (treesit-node-type parent))
                do (setq child parent parent (treesit-node-parent parent))
                finally return
                (cond (is-first
                       (setq +c-ts-mode--column-offset c-ts-mode-indent-offset)
                       (save-excursion (goto-char (treesit-node-start (treesit-node-child parent 0)))
  ...
  (defun +c-ts-mode--indent-style ()
    `(((n-or-p-is "preproc_arg") no-indent)
      ...
      ((is-named-child "alternative" "conditional_expression") (nth-sibling 2) 0)
      ...
      ;; int[5] a = { 0, 0, 0, 0 };
      ((+c-ts-mode--parent-is "initializer_list") +c-ts-mode--standalone-parent-or-uncle c-ts-mode-indent-offset)
      ;; Statement in enum.
  ...
  (after! dumb-jump
    (add-to-list 'dumb-jump-language-file-exts '(:language "c++" :ext "c"   :rgtype "cpp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "c++" :ext "C"   :rgtype "cpp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "c++" :ext "cc"  :rgtype "cpp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "c++" :ext "cpp" :rgtype "cpp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "c++" :ext "cxx" :rgtype "cpp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "c++" :ext "h"   :rgtype "cpp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "c++" :ext "H"   :rgtype "cpp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "c++" :ext "hh"  :rgtype "cpp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "c++" :ext "hpp" :rgtype "cpp"))
    (add-to-list 'dumb-jump-find-rules '(:type "function" :supports ("rg") :language "c++"
                                               :regex "\\w+[*\\s]+JJJ\\s*\\((?>[^()]+|(?1))*\\)[^;\\{]\\{"))
    (add-to-list 'dumb-jump-find-rules '(:type "define" :supports ("rg") :language "c++"
                                               :regex "^\s*#define\\s+JJJ\\b"))
    (add-to-list 'dumb-jump-find-rules '(:type "type" :supports ("rg") :language "c++"
...
(use-package jai-mode :elpaca (jai-mode :host github :repo "weirdNox/jai-mode"
  ...
  :config
  (after! dumb-jump
    (add-to-list 'dumb-jump-language-file-exts '(:language "jai" :ext "jai" :rgtype "jai"))
    (add-to-list 'dumb-jump-find-rules '(:type "constant" :supports ("rg") :language "jai"
                                               :regex "\\bJJJ\\s*::"))))
#+end_src
...
(use-package lisp-mode
  :config
  (after! dumb-jump
    (add-to-list 'dumb-jump-language-file-exts '(:language "lisp" :ext "el"    :rgtype "lisp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "lisp" :ext "el.gz" :rgtype "lisp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "lisp" :ext "lisp"  :rgtype "lisp"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "lisp" :ext "lsp"   :rgtype "lisp"))
    (add-to-list 'dumb-jump-find-rules '(:type "definition" :supports ("rg") :language "lisp"
                                               :regex "\\((cl-)?def(fun|macro|var|custom)\\s+JJJ\\j"))))
#+end_src
...
(use-package python
  ...
  (after! dumb-jump
    (add-to-list 'dumb-jump-language-file-exts '(:language "python" :ext "py" :rgtype "py"))
    (add-to-list 'dumb-jump-find-rules '(:type "assignment" :supports ("rg") :language "python"
                                               :regex "\\bJJJ\\s*=[^=\\n]"))
    (add-to-list 'dumb-jump-find-rules '(:type "function" :supports ("rg") :language "python"
                                               :regex "def\\s+JJJ\\b\\s*\\("))
    (add-to-list 'dumb-jump-find-rules '(:type "type" :supports ("rg") :language "python"
