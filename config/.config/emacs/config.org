...
(use-package projectile :straight t
  ...
  (defun +projectile--get-project-buffers (root)
    (when root
      (mapcar #'buffer-name
              (cl-remove-if-not
               (lambda (buffer)
                 (projectile-project-buffer-p buffer root))
               (buffer-list)))))
  ...
  (defun +projectile--get-relevant-projects (root)
    (cl-remove (if root (abbreviate-file-name root))
  ...
  (defun +projectile--sources (root-arg)
    (when-let (root (or root-arg (projectile-project-root) (+projectile--switch-project)))
  ...
  (defun +projectile (&optional root)
    (interactive)
    (when-let (buffer (consult--multi (+projectile--sources root)
                                      :prompt "Switch to: "
                                      :history 'projectile--project-history
                                      :require-match t
                                      :sort nil))
  ...
  (defun +bibtex-completion--prompt (&optional local-bib rebuild-cache)
    (when-let ((candidates (+bibtex-completion--get-formatted-candidates rebuild-cache))
               (chosen (consult--read candidates
                                      ...
                                      :category 'bibtex
                                      :require-match t
                                      :default +bibtex-completion--external-key)))
      (list (cdr (assoc chosen candidates)))))

  (defun nox/bibtex (arg)
    (interactive "P")
