  ...
  (setq!
   org-capture-templates '(("t" "task" entry (file "") "* NEXT %i%?")
                           ...
                           ("mb" "book" entry #'+capture-tracker-location #'+capture-tracker-template
                            :track-type book)
                           ("w" "web")
                           ("wb" "bookmark" plain #'+capture-bookmark-location #'+capture-bookmark-template)
...
See `+bibtex-assoc-pdf' for more information about associating with PDFs."
  ...
  (defun bibtex-autokey-get-year ()
    ...
    (let* ((yearfield (bibtex-autokey-get-field "year"))
           ...
           (year (cond ((> (length yearfield) 0) yearfield)
                       ((string-match "[0-9]+X*" date) (match-string 0 date)))))
...
(use-package ivy-bibtex :ensure
  ...
  (defun +bibtex*ivy-bibtex (orig-fun &optional arg local-lib)
    (let* ((local-bib (and (eq major-mode 'bibtex-mode)
       ...
       (lambda (field)
         (let* ((field (split-string field ":"))
           ...
           (when (string= field-name "title")
             (let ((maintitle (bibtex-completion-get-value "maintitle" entry)))
               (if (and maintitle field-value)
                   (setq field-value (concat maintitle ": " field-value))
                 (setq field-value (or maintitle field-value)))))
...
Return DEFAULT if FIELD is not present in ENTRY."
  ...
  (setq! bibtex-completion-bibliography +bibtex-bibliography
         ...
         bibtex-completion-additional-search-fields '("maintitle" "keywords")
