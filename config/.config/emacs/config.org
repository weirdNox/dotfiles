...
(use-package sh-mode
  ...
  :config
  (after! dumb-jump
    (add-to-list 'dumb-jump-language-file-exts '(:language "sh" :ext "sh"   :rgtype "sh"))
    (add-to-list 'dumb-jump-language-file-exts '(:language "sh" :ext "bash" :rgtype "sh"))
    (add-to-list 'dumb-jump-find-rules '(:type "function" :supports ("rg") :language "sh"
                                               :regex "(function\\s+JJJ\\b|\\bJJJ\\s*\\()"))
    (add-to-list 'dumb-jump-find-rules '(:type "variable" :supports ("rg") :language "sh"
                                               :regex "\\bJJJ=[^=]"))))
#+end_src
...
(use-package c-ts-mode
  ...
  (defsubst +c-ts-mode--skip-preproc (in-node)
    (cl-loop for node = in-node then (treesit-node-parent node)
             ...
             while (and node (string-match-p "preproc" type))
             finally return node))
