...
(use-package consult :straight t
  ...
  ;; NOTE(nox): Remove visible buffers from top suggestions in consult-buffer
  (consult--define-cache consult--cached-buffers
    (when-let (buffers (buffer-list))
      (cl-loop for window in (window-list)
               for buffer-in-window = (window-buffer window)
               do (setq buffers (nconc (delq buffer-in-window buffers) (list buffer-in-window))))
      buffers)))
#+end_src
...
"
  ...
  (defun +bibtex-completion--format-entry (entry width formats)
    (let* ((format (or (assoc-string (bibtex-completion-get-value "=type=" entry) formats 'case-fold)
                       (assoc t formats)))
             ...
             ((string= field-name "title")
              (let ((title (bibtex-completion-get-value "title" entry))
                ...
                (setq field-value
                      (or searchtitle
                          (concat maintitle
                                  (when (and maintitle title) ": ")
                                  title
                                  (when (and (or maintitle title) volume) ", ")
