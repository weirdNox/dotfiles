...
(use-package quelpa-use-package :ensure
  :demand
  :config (setq-default quelpa-use-package-inhibit-loading-quelpa t))
#+END_SRC
...
(gsetq abbrev-file-name               (concat nox-local-dir "abbrev.el")
       ...
       auto-save-list-file-name       (concat nox-cache-dir "autosave-list")
...
*** Modeline
#+BEGIN_SRC emacs-lisp
(use-package doom-modeline :quelpa (doom-modeline :repo "seagle0128/doom-modeline" :branch "develop"
                                                  :fetcher github)
  :config
...
(use-package dumb-jump :ensure
  ...
  :config
  (setq dumb-jump-default-project nox-emacs-dir
...
(use-package octave
  ...
  (setq-hook! inferior-octave-mode
    eldoc-documentation-function nil)

  ;; NOTE(nox): Remove whitespace from beginning when printing output
  (defun nox*org-babel-octave-evaluate-session (orig-func session body result-type &optional matlabp)
...
(defun nox-verify-refile-target ()
  (if (member (org-get-todo-state) org-done-keywords)
    ...
    t))
(gsetq org-refile-target-verify-function #'nox-verify-refile-target)
#+END_SRC
...
**** Entry
#+BEGIN_SRC emacs-lisp
(cl-defstruct +agenda-entry todo priority text tags low-effort marker project-status children)
...
(defun +agenda-project-get-prefix (taskp parent-continuations)
  (let ((prefix "  "))
    (if taskp
        (while parent-continuations
          (setq prefix (concat prefix
                               (if (cdr parent-continuations)
                                   (if (car parent-continuations) "|  " "   ")
                                 (propertize (if (car parent-continuations) "├⮚ " "╰⮚ ")
                                             'face
                                             (if (eq taskp 'low-effort)
                                                 '(:foreground "#b58900")
                                               nil)))))
          (setq parent-continuations (cdr parent-continuations)))
      (while parent-continuations
        (setq prefix (concat prefix
                             (if (cdr parent-continuations)
                                 (if (car parent-continuations) "|  " "   ")
...
(defun +agenda-project-printer (list &optional parent-continuations)
  (setq list (sort (+agenda-flatten-list list) #'+agenda-priority-sort))

  (if parent-continuations
      (while list
        (let ((entry (car list)))
          (unless (cdr list) (setf (car (last parent-continuations)) nil))
          ...
          (let ((org-todo-keyword-faces (if (+agenda-entry-project-status entry)
            ...
            (insert
             (+agenda-format-entry (+agenda-project-get-prefix (and (not (+agenda-entry-project-status entry))
                                                                    (or (and (+agenda-entry-low-effort entry)
                                                                             'low-effort)
                                                                        t))
                                                               parent-continuations)
                                   entry)))

          (+agenda-project-printer (+agenda-entry-children entry) (append parent-continuations (list t))))
        (setq list (cdr list)))

    (let ((first t)
          (org-todo-keyword-faces '(("NEXT" . '(:inherit org-todo :weight normal))
                                    ("TODO" . '(:inherit org-todo :weight normal)))))
      (dolist (entry list)
...
(defun +agenda-simple-printer (list)
  ...
  (dolist (entry list)
    (insert
     (+agenda-format-entry (if (+agenda-entry-low-effort entry)
    ...
    (when (or +agenda-show-private
      ...
      (if (not todo-type)
          (unless (member "TICKLER" (org-element-property :tags headline))
            (let* ((first-child (org-element-map (org-element-contents headline) 'headline #'identity
                                                 nil t 'headline))
                   (timestamp (or scheduled-ts deadline-ts
                                  (and contents-begin
                                       (progn
                                         ...
                                         (re-search-forward
                                          org-ts-regexp
                                          (or (and first-child (org-element-property :begin first-child))
                                              (org-element-property :end headline))
                                          t))
                                       (match-string 1)))))

              (if (and timestamp (< (org-time-stamp-to-now timestamp) -60))
        ...
        ;; NOTE(nox): This is a subtask, so at least it is stuck
        (unless (or (= +agenda-level 0) +agenda-project-status) (setq +agenda-project-status 'stuck))
        ...
        (if (or (eq todo-type 'done)
          ...
          (cond
           ...
           ((and (not (string= todo "NEXT")) scheduled-future)
            (unless (or (= +agenda-level 0) (eq +agenda-project-status 'next)) (setq +agenda-project-status 'planned))
           ...
           (t
            ...
            (unless (or (= +agenda-level 0) (eq +agenda-project-status 'next))
              (if project-status
                  (when (memq project-status '(next planned))
                    (setq +agenda-project-status project-status))
            ...
            (if project-status
              ...
              (if (= +agenda-level 0)
                ...
                (when (or (eq project-status 'next) (and (not project-status)
                                                         (or (string= todo "NEXT")
                                                             scheduled-past-or-now)))
                  (setq return entry))))))))
      return)))
          ...
          (org-with-wide-buffer
           ...
           (let ((+agenda-level 0)
             ...
             (setq +agenda-projects
                   (append (org-element-map ast 'headline #'+agenda-tasks-process-headline nil nil 'headline)
                           +agenda-projects))))))
