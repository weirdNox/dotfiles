...
(use-package quelpa :ensure
  :custom
  (quelpa-dir (concat nox-cache-dir "quelpa/"))
...
**** Files
#+BEGIN_SRC emacs-lisp
(setq-default abbrev-file-name               (concat nox-local-dir "abbrev.el")
...
**** Security
#+BEGIN_SRC emacs-lisp
(setq-default gnutls-verify-error (not (getenv "INSECURE"))
...
**** Theme hooks
#+BEGIN_SRC emacs-lisp
(defvar nox-load-theme-hook nil
  "Hooks to run when the theme is loaded")
(defun nox*load-theme-hooks (&rest _) (run-hooks 'nox-load-theme-hook))
(advice-add #'load-theme :after #'nox*load-theme-hooks)
#+END_SRC
...
*** Modeline
#+BEGIN_SRC emacs-lisp
(use-package doom-modeline :quelpa (doom-modeline :repo "seagle0128/doom-modeline" :fetcher github))
#+END_SRC
...
(use-package swiper :ensure
  :general ([remap swiper] #'counsel-grep-or-swiper)
  :config
...
(use-package org :ensure
  ...
  :custom
  (org-modules '(org-habit org-id org-protocol org-timer))

  ;; NOTE(nox): Directories
  ...
  ;; NOTE(nox): Tasks and states
  (org-todo-keywords '((sequence "TODO(t)" "NEXT(n)" "|" "DONE(d)")
                       (sequence "HOLD(h@/!)" "WAITING(w@/!)" "|" "CANCELLED(c@/!)")))
  (org-todo-state-tags-triggers '((todo ("CANCELLED"))
                                  (done ("HOLD") ("WAITING"))
                                  ("TODO" ("HOLD") ("WAITING"))
                                  ("NEXT" ("HOLD") ("WAITING"))
                                  ("DONE" ("CANCELLED"))
  ...
  (org-latex-packages-alist '(("" "tikz" t) ("american,siunitx,smartlabels" "circuitikz" t) ("" "mathtools" t)))
...
\\tikzset{every picture/.style={color=fg}}")
  ...
  (add-hook! 'org-after-refile-insert-hook (org-up-heading-safe) (org-update-statistics-cookies nil))
...
Else, return full list of projects."
  ...
  (defun nox|org-offer-all-agenda-tags () (setq-local org-complete-tags-always-offer-all-agenda-tags t))
...
(use-package org-attach
  :custom
  (org-attach-directory "Recursos/attach/")
...
(use-package org-noter :ensure
  ...
  :custom
  (org-noter-default-heading-title "Notas da página $p$")
...
(use-package calendar
  ...
  :config (setq-default calendar-date-display-form calendar-european-date-display-form))
