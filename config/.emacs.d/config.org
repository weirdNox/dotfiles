...
(use-package ob-octave
  ...
  (defun +babel-octave*trim-output (orig-func session body result-type &optional matlabp)
    (let ((result (funcall orig-func session body result-type matlabp)))
      (if (eq result-type 'output)
          (string-trim-left result)
        result)))
  (advice-add 'org-babel-octave-evaluate-session :around #'+babel-octave*trim-output))
#+END_SRC
