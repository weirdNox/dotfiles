...
(use-package doom-themes :ensure
  ...
  (nox-add-customize-theme-hook "^doom-"
    ...
    (nox-theme-set-faces `(org-special-keyword ((t . (:foreground ,(face-attribute 'shadow :foreground))))))))
...
(defun +ivy--get-buffer-display-and-name (buffer)
  (let ((buffer-name (buffer-name buffer))
    ...
    (unless (eq (aref buffer-name 0) ? )
      (if (setq directory (or (and file-name (file-name-directory file-name))
...
(defun +ivy/switch-buffer ()
  ...
  (let ((other-buffer (other-buffer (current-buffer) t)))
    (when (get-buffer-window other-buffer 'selected-frame)
...
(use-package bibtex
  ...
  (defvar +bibtex-notes          "~/Documents/Bibliography/Notes.org")
...
(use-package ivy-bibtex :ensure
  ...
  (setq! bibtex-completion-bibliography +bibtex-bibliography
         ...
         bibtex-completion-notes-template-one-file (format "* ${title} - ${year}
:PROPERTIES:
:Custom_ID: ${=key=}
