...
(use-package imenu-anywhere :ensure
  :general (nox-leader "i" 'ivy-imenu-anywhere)
  :config
  (defun +imenu-anywhere-filter (current other)
    (let ((current-mode (buffer-local-value 'major-mode current))
          (other-mode   (buffer-local-value 'major-mode other))
          projectile-require-project-root)
      (and (or (eq current-mode other-mode)
           ...
           (projectile-project-buffer-p other (file-truename (projectile-project-root))))))

  (push '(c-mode c++-mode) imenu-anywhere-friendly-modes)

  (setq! imenu-anywhere-delimiter ": "
         imenu-anywhere-buffer-filter-functions '(+imenu-anywhere-filter)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(defun +agenda*change-all-lines-fixface (&rest _)
