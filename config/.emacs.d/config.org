...
(use-package doom-themes :ensure
  ...
  (use-package doom-themes-treemacs
    ...
    :init
    (gsetq doom-treemacs-enable-variable-pitch nil))
...
(use-package ivy-rich :ensure
  :ghook 'ivy-mode-hook
  :config
  (gsetq ivy-rich-path-style 'abbrev
...
(use-package company-box :ensure
  ...
  :config
  (gsetq company-box-enable-icon nil))
#+END_SRC
...
(use-package cc-mode
  ...
  (gsetq c-default-style "weirdNox")
...
(use-package org :ensure
  ...
  :init
  (gsetq org-directory "~/Essential/Org/")
  ...
  (gsetq
   org-capture-templates '(("t" "Tarefa" entry (file "")
                           ...
                           ("d" "Diário" entry (file+olp+datetree nox-org-journal-file)
                            "* %?" :clock-in t :clock-resume t)
                           ("f" "Tracking")
                           ("ff" "Filme" entry (file+olp nox-org-tracker-file "Filmes")
                            "* WATCH %?" :prepend t)
                           ("fs" "Série" entry (file+olp nox-org-tracker-file "Séries")
                            "* WATCH %?" :prepend t)
                           ("fl" "Livro" entry (file+olp nox-org-tracker-file "Livros")
                            "* READ %?"  :prepend t)
                           ("w" "Web")
                           ("wb" "Bookmark" plain (function +capture-bookmark-find-buffer)
                            (function +capture-bookmark-get-template))
...
(use-package org-ref :ensure
  ...
  :demand
  :config
  (gsetq org-ref-default-bibliography '("~/Documents/Bibliography/References.bib")
...
(use-package ivy-bibtex :ensure
  :config
  (gsetq bibtex-completion-notes-template-one-file (format "* ${title} - ${year}
:PROPERTIES:
:Custom_ID: ${=key=}
...
"
  ...
  (defun nox*disable-completion-notes-mode (&optional arg) (widen))
...
(use-package deft :ensure
  ...
  :config
  (gsetq deft-directory (concat org-directory "Logbook/")
...
(use-package nov :ensure
  :mode (("\\.epub\\'" . nov-mode))
  :config
  (gsetq nov-save-place-file (concat nox-cache-dir "nov-places")))
#+END_SRC
...
(use-package eshell
  :config
  (gsetq eshell-directory-name (concat nox-cache-dir "eshell/")))
#+END_SRC
...
(use-package treemacs :ensure
  :config
  (gsetq treemacs-collapse-dirs (if (executable-find "python") 3 0)
