...
*** Font
#+BEGIN_SRC emacs-lisp
(defvar nox-fonts '(("Source Code Pro" . 10) ("PragmataPro" . 12) ("Iosevka" . 12)
                    ("DejaVu Sans Mono" . 10) ("Inconsolata" . 13))
...
(cl-defstruct +agenda-entry
  todo priority text tags planned low-effort marker is-tickler schedule-or-deadline-to-now
  project-status children)

(defun +agenda-entry (headline &optional tags schedule-or-deadline)
  (let ((todo-type (org-element-property :todo-type headline))
    ...
    (make-+agenda-entry
     ...
     :schedule-or-deadline-to-now (and schedule-or-deadline (org-time-stamp-to-now schedule-or-deadline)))))
...
(defun +agenda-priority-sort (a b)
  (let ((pa (or (+agenda-entry-priority a) org-default-priority))
        (ta (or (+agenda-entry-schedule-or-deadline-to-now a) 0))
        (pb (or (+agenda-entry-priority b) org-default-priority))
        (tb (or (+agenda-entry-schedule-or-deadline-to-now b) 0)))
    (or(< pa pb) (and (= pa pb) (< ta tb)))))
...
(defun +agenda-tasks-process-headline (headline)
  (when (or +agenda-show-private (not (member "PRIVATE" (org-element-property :tags headline))))
    (let* ((scheduled-ts (org-element-property :raw-value (org-element-property :scheduled headline)))
           ...
           (tags-list (cl-remove-duplicates (append (org-element-property :tags headline) +agenda-parent-tags)
                                            :test 'string=))
           (entry (+agenda-entry headline tags-list has-scheduling))
      ...
      (if (not todo-type)
          ...
          (let* ((time-to-now (+agenda-entry-schedule-or-deadline-to-now entry))
