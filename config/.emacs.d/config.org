...
*** Font
#+BEGIN_SRC emacs-lisp
(defvar nox-fonts '(("PragmataPro" . 12) ("Iosevka" . 12) ("DejaVu Sans Mono" . 11)
...
(use-package doom-themes :ensure
  ...
  (nox-add-customize-theme-hook "^doom-"
    (nox-theme-set-faces '(outline-1 ((t (:height 1.0))))
...
*** Modeline
Thanks to @seagle0128 for creating the =doom-modeline= package, on which this is based!

#+BEGIN_SRC emacs-lisp
(require 'all-the-icons)
(use-package eldoc-eval :ensure :demand)
(require 'projectile)
(use-package shrink-path :ensure :demand)
...
(defvar +modeline-bar-width 1
  "How wide the mode-line bar should be (only respected in GUI Emacs).")

(defvar +modeline-buffer-file-name-style 'truncate-upto-project
  "Determines the style used by `+modeline-buffer-file-name'.
...
(defface +modeline-project-root-dir
  '((t (:inherit (mode-line-emphasis bold))))
...
(defface +modeline-eldoc-bar '((t (:inherit shadow)))
  "The face used for the left-most bar on the mode-line when eldoc-eval is active."
  :group 'mode-line-faces)

(defface +modeline-inactive-bar '((t (:inherit warning :inverse-video t)))
...
(defmacro +modeline-def-segment (name &rest body)
  ...
  (let ((sym (intern (format "+modeline-segment--%s" name)))
        (docstring (if (stringp (car body))
                       (pop body)
                     (format "%s modeline segment" name))))
    (cond ((and (symbolp (car body))
                (not (cdr body)))
           (add-to-list '+modeline-var-alist (cons name (car body)))
           `(add-to-list '+modeline-var-alist (cons ',name ',(car body))))
          (t
           (add-to-list '+modeline-fn-alist (cons name sym))
           `(progn
...
(defsubst +modeline--prepare-segments (segments)
  ...
  (let (forms it)
    (dolist (seg segments)
      (cond ((stringp seg)
             (push seg forms))
            ((symbolp seg)
             (cond ((setq it (cdr (assq seg +modeline-fn-alist)))
                    (push (list it) forms))
                   ((setq it (cdr (assq seg +modeline-var-alist)))
                    (push it forms))
                   ((error "%s is not a defined segment" seg))))
            ((error "%s is not a valid segment" seg))))
    (nreverse forms)))

(defun +modeline-def-modeline (name lhs &optional rhs)
  "Defines a modeline format and byte-compiles it.

  NAME is a symbol to identify it (used by `+modeline' for retrieval).
  LHS and RHS are lists of symbols of modeline segments defined with
  `+modeline-def-segment'.

  Example:
  (+modeline-def-modeline 'minimal
                              '(bar matches \" \" buffer-info)
                              '(media-info major-mode))
  (+modeline-set-modeline 'minimal t)"
  (let ((sym (intern (format "+modeline-format--%s" name)))
        (lhs-forms (+modeline--prepare-segments lhs))
        (rhs-forms (+modeline--prepare-segments rhs)))
    (defalias sym
      (lambda ()
...
(defun +modeline (key)
  ...
  (let ((fn (intern-soft (format "+modeline-format--%s" key))))
    (when (functionp fn)
      `(:eval (,fn)))))

(defun +modeline-set-modeline (key &optional default)
  "Set the modeline format. Does nothing if the modeline KEY doesn't exist.
If DEFAULT is non-nil, set the default mode-line for all buffers."
  (when-let ((modeline (+modeline key)))
    (setf (if default
              (default-value 'mode-line-format)
            (buffer-local-value 'mode-line-format (current-buffer)))
          (list "%e" modeline))))

(defsubst +modeline-project-root ()
  "Get the path to the root of your project."
  (let (projectile-require-project-root) (projectile-ensure-project (projectile-project-root))))
...
(defun +modeline-eldoc (text)
  "Get eldoc TEXT for mode-line."
  (concat (when (display-graphic-p)
            (+modeline--make-xpm '+modeline-eldoc-bar +modeline-height +modeline-bar-width))
          text))

;; Show eldoc in the mode-line with `eval-expression'
(defun +modeline--show-eldoc (input)
  "Display string INPUT in the mode-line next to minibuffer."
  (with-current-buffer (eldoc-current-buffer)
    (let* ((str              (and (stringp input) input))
           (mode-line-format (or (and str (or (+modeline-eldoc str) str))
                                 mode-line-format))
           mode-line-in-non-selected-windows)
      (force-mode-line-update)
      (sit-for eldoc-show-in-mode-line-delay))))
(setq eldoc-in-minibuffer-show-fn #'+modeline--show-eldoc)
...
(defun +modeline-unset-selected-window ()
  "Unset `+modeline-current-window' appropriately."
...
(defun +modeline-maybe-icon-octicon (&rest args)
  "Display octicon via `ARGS'."
  (when (display-graphic-p)
    (apply 'all-the-icons-octicon args)))

(defun +modeline-maybe-icon-faicon (&rest args)
  "Display font awesome icon via `ARGS'."
  (when (display-graphic-p)
    (apply 'all-the-icons-faicon args)))

(defun +modeline-maybe-icon-material (&rest args)
...
(defun +modeline--make-xpm (face width height)
...
(defun +modeline-buffer-file-name ()
  "Propertized variable `buffer-file-name' based on `+modeline-buffer-file-name-style'."
  (let ((buffer-file-name (or (buffer-file-name (buffer-base-buffer)) "")))
    (unless buffer-file-truename (setq buffer-file-truename (file-truename buffer-file-name)))
    (propertize
     (pcase +modeline-buffer-file-name-style
       (`truncate-upto-project (+modeline--buffer-file-name buffer-file-name buffer-file-truename 'shrink))
       (`truncate-upto-root    (+modeline--buffer-file-name-truncate buffer-file-name buffer-file-truename))
       (`truncate-all          (+modeline--buffer-file-name-truncate buffer-file-name buffer-file-truename t))
       (`relative-to-project   (+modeline--buffer-file-name-relative buffer-file-name buffer-file-truename))
       (`relative-from-project (+modeline--buffer-file-name-relative buffer-file-name buffer-file-truename
                                                                     'include-project))
       (`file-name (propertize (file-name-nondirectory buffer-file-name)
                               'face
                               (let ((face (or (and (buffer-modified-p) '+modeline-buffer-modified)
                                               (and (+modeline--active) '+modeline-buffer-file))))
                                 (when face `(:inherit ,face))))))
     'help-echo buffer-file-truename)))

(defun +modeline--buffer-file-name-truncate (file-path true-file-path &optional truncate-tail)
  "Propertized `buffer-file-name' that truncates every dir along path.
If TRUNCATE-TAIL is t also truncate the parent directory of the file."
  (let ((dirs (shrink-path-prompt (file-name-directory true-file-path)))
        (active (+modeline--active)))
    (if (null dirs)
        (propertize "%b" 'face (if active '+modeline-buffer-file))
      (let ((modified-faces (if (buffer-modified-p) '+modeline-buffer-modified)))
        (let ((dirname (car dirs))
              (basename (cdr dirs))
              (dir-faces (or modified-faces (if active '+modeline-project-root-dir)))
              (file-faces (or modified-faces (if active '+modeline-buffer-file))))
...
(defun +modeline--buffer-file-name-relative (_file-path true-file-path &optional include-project)
  "Propertized `buffer-file-name' showing directories relative to project's root only."
  (let ((root (+modeline-project-root))
        (active (+modeline--active)))
    (if (null root)
        (propertize "%b" 'face (if active '+modeline-buffer-file))
      (let* ((modified-faces (if (buffer-modified-p) '+modeline-buffer-modified))
             ...
             (relative-faces (or modified-faces (if active '+modeline-buffer-path)))
             (file-faces (or modified-faces (if active '+modeline-buffer-file))))
...
(defun +modeline--buffer-file-name (file-path _true-file-path &optional truncate-project-root-parent)
  "Propertized `buffer-file-name'.
If TRUNCATE-PROJECT-ROOT-PARENT is t space will be saved by truncating it down fish-shell style.

Example: ~/Projects/FOSS/emacs/lisp/comint.el => ~/P/F/emacs/lisp/comint.el"
  (let* ((project-root (+modeline-project-root))
...
(+modeline-def-segment buffer-default-directory
  "Displays `default-directory'. This is for special buffers like the scratch
buffer where knowing the current project directory is important."
  (let ((face (if (+modeline--active) '+modeline-buffer-path)))
    (concat (if (display-graphic-p) " ")
            (+modeline-maybe-icon-octicon
             "file-directory"
             :face face
             :v-adjust -0.05
             :height 1.25)
...
directory, the file name, and its state (modified, read-only or non-existent)."
  (concat (cond (buffer-read-only
                 (concat (+modeline-maybe-icon-octicon
                          "lock"
                          :face '+modeline-warning
                          :v-adjust -0.05)
                         " "))
                ((buffer-modified-p)
                 (concat (+modeline-maybe-icon-faicon
                          "floppy-o"
...
(+modeline-def-segment buffer-info-simple
  "Display only the current buffer's name, but with fontification."
  (propertize
   "%b"
   'face (cond ((and buffer-file-name (buffer-modified-p))
                '+modeline-buffer-modified)
               ((+modeline--active) '+modeline-buffer-file))))
...
(+modeline-def-segment major-mode
  ...
  (propertize (concat (format-mode-line mode-name)
                      (and (boundp 'text-scale-mode-amount)
                           ...
                           (format (if (> text-scale-mode-amount 0) " (%+d)" " (%-d)") text-scale-mode-amount)))
              'face (and (+modeline--active) '+modeline-buffer-major-mode)))
...
(defun +modeline--update-vcs (&rest _)
              ...
              (concat (if (display-graphic-p) "  ")
                      (cond ((memq state '(edited added))
                            ...
                            ((eq state 'needs-merge)
                             (if active (setq face '+modeline-info))
                             (+modeline-maybe-icon-octicon "git-merge" :face face))
                            ((eq state 'needs-update)
                             (if active (setq face '+modeline-warning))
                             (+modeline-maybe-icon-octicon "arrow-down" :face face))
                            ((memq state '(removed conflict unregistered))
                             (if active (setq face '+modeline-urgent))
                             (+modeline-maybe-icon-octicon "alert" :face face))
                            (t
                             (if active (setq face '+modeline-info))
                             (+modeline-maybe-icon-octicon
                              "git-branch"
                      ...
                      (propertize (substring vc-mode (+ (if (eq backend 'Hg) 2 3) 2))
                                  'face (if active face))
                      " "))))))
(add-hook 'after-revert-hook #'+modeline--update-vcs)
(add-hook 'after-save-hook #'+modeline--update-vcs)
(add-hook 'find-file-hook #'+modeline--update-vcs t)
(advice-add #'select-window :after #'+modeline--update-vcs)

(+modeline-def-segment vcs
  "Displays the current branch, colored based on its state."
  (if (+modeline--active) +modeline--vcs ""))
...
(defvar-local modeline-enable-word-count t
  "If non-nil, a word count will be added to the selection-info modeline segment.")

(+modeline-def-segment selection-info
  "Information about the current selection, such as how many characters and
lines are selected, or the NxM dimensions of a block selection."
  (when (and mark-active (+modeline--active))
    (let ((beg (region-beginning)) (end (region-end)))
      (propertize (let ((lines (count-lines beg (min end (point-max)))))
                    (concat (cond ((bound-and-true-p rectangle-mark-mode)
                                  ...
                                  ((format "%dC" (- end beg))))
                            (when modeline-enable-word-count (format " %dW" (count-words beg end)))))
...
(defun +modeline--macro-recording ()
  ...
  (when (and (+modeline--active) (or defining-kbd-macro executing-kbd-macro))
    (let ((sep (propertize " " 'face '+modeline-panel)))
      (concat sep (propertize "Macro" 'face '+modeline-panel)
              sep (+modeline-maybe-icon-octicon "triangle-right" :face '+modeline-panel :v-adjust -0.05)
              sep))))
...
(defsubst +modeline--multiple-cursors ()
  ...
  (when (bound-and-true-p multiple-cursors-mode)
    (propertize
     (concat (car mc/mode-line) (eval (cadadr mc/mode-line)) " ")
     'face (if (+modeline--active) '+modeline-panel))))

(+modeline-def-segment matches
  "Displays:
- The currently recording macro
- Number of multiple cursors"
  (let ((meta (concat (+modeline--macro-recording) (+modeline--multiple-cursors))))
...
(+modeline-def-segment media-info
  "Metadata regarding the current file, such as dimensions for images."
  (cond ((eq major-mode 'image-mode)
         (cl-destructuring-bind (width . height) (image-size (image-get-display-property) :pixels)
...
(+modeline-def-segment global
  "For the time string and whatever uses global-mode-string."
  (if (< 0 (length global-mode-string)) '(" " global-mode-string "  ") ""))
...
(defvar-local +modeline-column-zero-based
  (or (bound-and-true-p column-number-indicator-zero-based) t)
  "When non-nil, mode line displays column numbers zero-based.
See `column-number-indicator-zero-based'.")

(defvar-local +modeline-percent-position
...
(setq-default mode-line-position
              '((line-number-mode
                 (column-number-mode
                  (+modeline-column-zero-based " %l:%c" " %l:%C")
                  " %l")
                 (column-number-mode (+modeline-column-zero-based " :%c" " :%C")))
                (if +modeline-percent-position (" " +modeline-percent-position))
                (:eval (when (or line-number-mode column-number-mode +modeline-percent-position) " "))))
...
(+modeline-def-modeline 'main
                        '(bar matches " " buffer-info buffer-position " " selection-info)
                        '(global buffer-encoding major-mode process vcs))

(+modeline-def-modeline 'minimal
                        '(bar matches " " buffer-info)
                        '(media-info major-mode))

(+modeline-def-modeline 'special
                        '(bar matches " " buffer-info-simple buffer-position " " selection-info)
                        '(global buffer-encoding major-mode process))

(+modeline-def-modeline 'project
                        '(bar buffer-default-directory)
                        '(global major-mode))

(+modeline-def-modeline 'media
                        '(bar " %b  ")
...
(defun +modeline-init ()
  ...
  (setq +modeline--bar-active   (+modeline--make-xpm '+modeline-bar          +modeline-bar-width +modeline-height)
        +modeline--bar-inactive (+modeline--make-xpm '+modeline-inactive-bar +modeline-bar-width +modeline-height))
  (unless after-init-time
    (dolist (bname '("*scratch*" "*Messages*")) ;; Setup already created buffers
      (with-current-buffer bname (+modeline-set-modeline 'main)))))

;;;###autoload
(defun +modeline-set-special-modeline ()
  "Set sepcial mode-line."
  (+modeline-set-modeline 'special))

;;;###autoload
(defun +modeline-set-media-modeline ()
  "Set media mode-line."
  (+modeline-set-modeline 'media))

;;;###autoload
(defun +modeline-set-project-modeline ()
  "Set project mode-line."
  (+modeline-set-modeline 'project))

;;
;; Bootstrap
;;

(+modeline-set-modeline 'main t)

(add-hook 'image-mode-hook #'+modeline-set-media-modeline)
(add-hook 'org-src-mode-hook #'+modeline-set-special-modeline)
...
(use-package counsel :ensure
  ...
  (:keymaps 'ivy-minibuffer-map
            "<return>" 'ivy-alt-done
            "C-j" 'ivy-done)
  (:keymaps 'ivy-minibuffer-map "C-r" 'counsel-minibuffer-history)
  ...
  (let ((first t) entry)
    (while list
      ...
      (cond
       ...
       ((or (+agenda-entry-planned entry) (eq (+agenda-entry-project-status entry) 'planned))
            ...
            (cond
             ...
             (t
              ...
              (let* ((+agenda-project-status nil)
                ...
                ;; NOTE(nox): When this project is not planned, we need to remove its planned tasks and
                ;; insert them in the planned list. A stuck project doesn't have any planned children so,
                ;; for this check, not planned ≡ next.
...
(use-package ediff
  ...
  (defun ediff|save-wconf () (setq ediff--saved-wconf (current-window-configuration)))
  (defun ediff|restore-wconf () (set-window-configuration ediff--saved-wconf))
