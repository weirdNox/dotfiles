...
(setq!
 ...
 `((:eval (let ((root (projectile-project-root))
                (inherit-faces (if (and buffer-file-name (buffer-modified-p))
                                   '(error bold) 'mode-line-buffer-id)))
            (concat (and root (concat "[" (file-name-nondirectory (directory-file-name root)) "]  "))
                    (propertize "%b" 'face inherit-faces)))))
...
(defun +compile (arg)
  ...
  (let ((chosen-cons (and +compile-info (cons (+compile-info-path +compile-info) +compile-info)))
    ...
    (let* ((info (if (and set-default (> (length cons-list) 1))
      ...
      (setq command-args (replace-regexp-in-string (regexp-quote "%f") (shell-quote-argument calling-file-name)
                                                   command-args))
...
(use-package magit :ensure
  ...
  (magit-define-popup-switch 'magit-merge-popup
    ?h "Allow unrelated histories" "--allow-unrelated-histories"))
#+END_SRC
